...
//parameters
float a [from_range][to_range][risk_category]=...; //availability matrix
...
forall (j in to_range,k in risk_category) sum(i in from_range) a[i][j][k]*y[i] >= r[j][k]*n[k]; //Each district is covered
